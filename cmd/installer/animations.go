...
// TypewriterTicker types out messages one character at a time
type TypewriterTicker struct {
	messages      []string
	messagesRunes [][]rune // Pre-converted to runes for proper Unicode handling
	messageIndex  int
...
// NewTypewriterTicker creates a new typewriter ticker with the given messages
func NewTypewriterTicker(messages []string) *TypewriterTicker {
	// Shuffle messages
	...
	// Pre-convert all messages to runes for proper Unicode handling
	messagesRunes := make([][]rune, len(shuffled))
	for i, m := range shuffled {
		messagesRunes[i] = []rune(m)
	}

	return &TypewriterTicker{
		messages:      shuffled,
		messagesRunes: messagesRunes,
		messageIndex:  0,
		charIndex:     0,
		lastUpdate:    time.Now(),
...
// Update advances the ticker animation
func (t *TypewriterTicker) Update() {
	...
	if t.paused {
		...
		t.messageIndex = (t.messageIndex + 1) % len(t.messages)
		t.charIndex = 0
		t.paused = false
		t.lastUpdate = now
		return
	...
	if now.Sub(t.lastUpdate) >= t.charDelay {
		...
		currentRunes := t.messagesRunes[t.messageIndex]
...
// Render returns the current ticker text
func (t *TypewriterTicker) Render(width int) string {
	...
	if t.paused {
		result = t.messages[t.messageIndex]
	} else {
