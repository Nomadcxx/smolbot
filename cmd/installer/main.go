...
func startSignalLinkCmd() tea.Cmd {
	return func() tea.Msg {
		...
		linker := NewSignalLinker("signal-cli", dataDir)
		if err := linker.Start(); err != nil {
			return signalInitResult{err: err}
		}
		return signalInitResult{linker: linker}
	}
}

func signalPollCmd() tea.Cmd {
	return tea.Tick(200*time.Millisecond, func(t time.Time) tea.Msg {
...
func (m model) handleSignalSetupKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	switch msg.String() {
	case "enter":
		if m.signalDone {
			if m.whatsappEnabled {
		...
		return m, nil
	case "esc":
		if m.signalLinker != nil {
			m.signalLinker.Cleanup()
			m.signalLinker = nil
		}
		m.signalDone = false
		m.signalQRCode = ""
...
func (m model) handleSignalPoll(msg signalPollMsg) (tea.Model, tea.Cmd) {
	...
	qr, status, account, done, err := m.signalLinker.Poll()
	if qr != "" {
		m.signalQRCode = qr
	}
	if status != "" {
		m.signalStatus = status
	}
	if account != "" {
		m.signalAccount = account
	}
	if done {
		m.signalDone = true
		if err != nil {
			m.signalError = err.Error()
			m.signalLinker.Cleanup()
			m.signalLinker = nil
		}
		return m, nil
